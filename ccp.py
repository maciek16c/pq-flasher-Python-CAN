...
class CcpClient():
  def __init__(self, bus, tx_addr: int, rx_addr: int, byte_order: BYTE_ORDER=BYTE_ORDER.BIG_ENDIAN, debug=False):
  ...
  def _send_cro(self, cmd: int, dat: bytes = b"") -> None:
    ...
    message = can.Message(arbitration_id=self.tx_addr, data=tx_data, is_extended_id=False)
  ...
  def _recv_dto(self, timeout: float) -> bytes:
    ...
    while time.time() - start_time < timeout:
        ...
        if message is not None and message.arbitration_id == self.rx_addr:
            rx_data = message.data
            if self.debug:
                print(f"CAN-RX: {hex(message.arbitration_id)} - 0x{rx_data.hex()}")
