...
class TP20Transport:
	def __init__(self, bus, module: int, timeout: float = 0.1, debug: bool = True):
	...
	def can_recv(self, addr: Optional[int] = None) -> bytes:
		...
		while time.monotonic() - start_time < self.timeout:
			...
			# Odbieranie wiadomości z biblioteki python-can
			msg = self.bus.recv()
	...
	def can_send(self, dat: bytes, addr: Optional[int] = None):
		...
		# Przygotowanie wiadomości CAN
		message = can.Message(arbitration_id=addr, data=dat, is_extended_id=False)
